...
for line in sys.stdin:
    ...
    count_str, is_direct_str, location = [x.strip() for x in data.split(',')]
    count = int(count_str)
    is_direct = int(is_direct_str)

    user, recommended_friend = [x.strip() for x in pair.split(',')]

    print(f"{user}\t{recommended_friend},{count},{is_direct},{location}")
    print(f"{recommended_friend}\t{user},{count},{is_direct},{location}")